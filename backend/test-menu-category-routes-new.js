...
async function testMenuCategoryRoutes() {
    try {
        console.log('Testing Menu Category Routes (New)...');
        
        // Test GET /api/categories - Get all menu categories
        console.log('\n--- Testing GET /api/categories ---');
        try {
            const getResponse = await axios.get('http://localhost:3005/api/categories');
        ...
        try {
            const postResponse = await axios.post('http://localhost:3005/api/categories', {

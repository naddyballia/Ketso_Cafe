...
async function testNewApp() {
    try {
        ...
        const rootResponse = await axios.get('http://localhost:3003');
        ...
        // Test GET request to /api/categories on new app
        console.log('\nTesting GET /api/categories on new app...');
        try {
        ...
        // Test POST request to /api/categories on new app
        console.log('\nTesting POST /api/categories on new app...');
        try {
            const postResponse = await axios.post(
                'http://localhost:3003/api/categories',
                { name: 'New Test Category', restaurant_id: 1 },
                { headers: { Authorization: `Bearer ${token}` } }
            );
